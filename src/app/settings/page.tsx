...
export default function SettingsPage() {
  ...
  // Escape closes edit dialog
  useEffect(() => {
    if (!editState) return
    const fn = (e: KeyboardEvent) => { if (e.key === 'Escape') setEditState(null) }
    ...
    try {
      ...
      if (editState.type === 'EPF') {
        res = await fetch('/api/epf', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            employeeMonthly: editState.employeeMonthly,
            employerMonthly: editState.employerMonthly,
            dayOfMonth:      editState.dayOfMonth,
          }),
        })
  ...
  async function exportStocks() {
    ...
    try {
      ...
      setCsvDone('stocks')
      setTimeout(() => setCsvDone(null), 2000)
    } finally {
      setCsvLoading(null)
    }
  }
  ...
  async function exportFDRD() {
    ...
    try {
      ...
      const fdRows = (fds as Array<Record<string, unknown>>).map(f => ({
        Type: 'FD', Name: f.name, Bank: f.bankName, Platform: f.platform,
        'Principal (₹)': f.principal, 'Interest Rate': f.interestRate,
        'Current Value (₹)': f.currentValue, 'Maturity Value (₹)': f.maturityValue,
        'Start Date': f.startDate, 'Maturity Date': f.maturityDate,
      }))
      const rdRows = (rds as Array<Record<string, unknown>>).map(r => ({
        Type: 'RD', Name: r.name, Bank: r.bankName, Platform: r.platform,
        'Principal (₹)': r.totalInvested, 'Interest Rate': r.interestRate,
        'Current Value (₹)': r.currentValue, 'Maturity Value (₹)': r.maturityValue,
        'Start Date': r.startDate, 'Maturity Date': r.maturityDate,
      }))
      downloadCSV([...fdRows, ...rdRows], `ledger-fdrd-${dateStr}.csv`)
      setCsvDone('fdrd')
      setTimeout(() => setCsvDone(null), 2000)
    } finally {
      setCsvLoading(null)
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {rulesLoading ? (
                  ...
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{ padding: '14px 22px', borderBottom: '0.5px solid var(--color-border-subtle)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <div style={{ height: 14, width: '40%', borderRadius: '4px', background: 'var(--color-surface-raised)', animation: 'pulse 1.5s ease-in-out infinite' }} />
                        <div style={{ height: 12, width: '65%', borderRadius: '4px', background: 'var(--color-surface-raised)', animation: 'pulse 1.5s ease-in-out infinite' }} />
                      </div>
                      <div style={{ width: 60, height: 22, borderRadius: '4px', background: 'var(--color-surface-raised)', animation: 'pulse 1.5s ease-in-out infinite', flexShrink: 0 }} />
                    </div>
                  ))}
                </>
              ) : rules.length === 0 ? (
                ...
                rules.map((rule, i) => (
                  <div
                    ...
                    style={{
                      padding: '14px 22px',
                      borderBottom: i < rules.length - 1 ? '0.5px solid var(--color-border-subtle)' : 'none',
                  ...
                  >
                    ...
                    <span style={{
                      ...
                      background: rule.status === 'ACTIVE' ? '#F0FDF4' : 'var(--color-surface-raised)',
                      color: rule.status === 'ACTIVE' ? '#16A34A' : 'var(--color-text-muted)',
                      flexShrink: 0,
                    }}>
                      {rule.status}
                    </span>
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginBottom: '14px', lineHeight: 1.4 }}>
                      {desc}
                    </div>
                    <button
                      onClick={fn}
          ...
          {activeSection === 'danger' && (
            <div style={{ border: '0.5px solid #FECDD3', borderRadius: '12px', overflow: 'hidden' }}>
              ...
              {/* Reset stocks */}
              <DangerRow
                name="Reset all stocks"
                description="Deletes all stocks and transaction history"
                successMessage={dangerSuccess['stocks']}
                confirming={dangerConfirm === 'stocks'}
                loading={dangerLoading && dangerConfirm === 'stocks'}
                onTrigger={() => setDangerConfirm('stocks')}
                onCancel={() => setDangerConfirm(null)}
                onConfirm={() => handleDanger('stocks')}
                borderBottom
              />

              {/* Reset mutual funds */}
              <DangerRow
                name="Reset all mutual funds"
                description="Deletes all funds, transactions and SIP configs"
                successMessage={dangerSuccess['mf']}
                confirming={dangerConfirm === 'mf'}
                loading={dangerLoading && dangerConfirm === 'mf'}
                onTrigger={() => setDangerConfirm('mf')}
                onCancel={() => setDangerConfirm(null)}
                onConfirm={() => handleDanger('mf')}
                borderBottom
              />
              ...
              <div style={{ padding: '16px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginTop: '2px' }}>
                    Permanently deletes all assets, transactions and snapshots
                  </div>
                </div>
                <div style={{ flexShrink: 0 }}>
                  {dangerConfirm === 'all' ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      />
                      <button
                        onClick={() => { setDangerConfirm(null); setResetText('') }}
                        style={{ padding: '5px 10px', borderRadius: '6px', border: '0.5px solid var(--color-border)', background: 'transparent', color: 'var(--color-text-muted)', fontSize: '12px', fontFamily: 'inherit', cursor: 'pointer', whiteSpace: 'nowrap' }}
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        style={{
                          ...
                          color: '#DC2626', fontSize: '12px', fontFamily: 'inherit',
                          cursor: resetText === 'RESET' && !dangerLoading ? 'pointer' : 'default',
                          opacity: resetText === 'RESET' && !dangerLoading ? 1 : 0.5,
                          whiteSpace: 'nowrap',
                        }}
                      >
                    ...
                    <button
                      onClick={() => setDangerConfirm('all')}
                      style={{ padding: '6px 14px', borderRadius: '6px', border: '0.5px solid #FCA5A5', background: '#FEE2E2', color: '#DC2626', fontSize: '12.5px', fontFamily: 'inherit', cursor: 'pointer', whiteSpace: 'nowrap' }}
                    >
                      Reset everything
                    </button>
                  )}
          ...
          >
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      value={editState.employeeMonthly}
                      onChange={e => setEditState({ ...editState, employeeMonthly: +e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number" min={0} style={inputStyle}
                      value={editState.employerMonthly}
                      onChange={e => setEditState({ ...editState, employerMonthly: +e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEditState({ ...editState, dayOfMonth: Math.min(28, Math.max(1, +e.target.value)) })}
                    />
                  </div>
              ...
              {editState.type === 'RD' && (
                <>
                  <div>
                    ...
                    <input
                      type="number" style={{ ...inputStyle, background: 'var(--color-surface-raised)', color: 'var(--color-text-muted)' }}
                      value={editState.monthlyAmount} readOnly
                    />
                  </div>
                  <div>
                    <label style={labelStyle}>Day of month (1–28)</label>
                      ...
                      {(['ACTIVE', 'PAUSED'] as const).map(s => (
                        <button
                          ...
                          style={{
                            ...
                            border: `0.5px solid ${editState.status === s ? 'var(--color-text-primary)' : 'var(--color-border)'}`,
                            background: editState.status === s ? 'var(--color-text-primary)' : 'transparent',
                            color: editState.status === s ? 'var(--color-surface)' : 'var(--color-text-secondary)',
            ...
            <div style={{ padding: '12px 20px', borderTop: '0.5px solid var(--color-border)', display: 'flex', gap: '8px' }}>
              ...
              <button
                ...
                style={{ flex: 1, padding: '8px', borderRadius: '6px', border: 'none', background: 'var(--btn-primary-bg)', color: 'var(--btn-primary-text)', fontSize: '13px', fontWeight: 500, fontFamily: 'inherit', cursor: editLoading ? 'default' : 'pointer', opacity: editLoading ? 0.7 : 1 }}
              >
...
function DangerRow({
  ...
  return (
    <div style={{
      padding: '16px 22px',
      borderBottom: borderBottom ? '0.5px solid #FEF2F2' : 'none',
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {successMessage && (
          <div style={{ fontSize: '12px', color: '#16A34A', marginTop: '4px', fontWeight: 500 }}>{successMessage}</div>
        )}
      </div>
      <div style={{ flexShrink: 0 }}>
        {confirming ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button
              onClick={onConfirm}
              disabled={loading}
              style={{ padding: '5px 10px', borderRadius: '6px', border: '0.5px solid #FECDD3', background: '#FFF5F5', color: '#DC2626', fontSize: '12px', fontFamily: 'inherit', cursor: loading ? 'default' : 'pointer', whiteSpace: 'nowrap' }}
            >
          ...
          <button
            onClick={onTrigger}
            style={{ padding: '6px 14px', borderRadius: '6px', border: '0.5px solid #FECDD3', background: '#FFF5F5', color: '#DC2626', fontSize: '12.5px', fontFamily: 'inherit', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            {name}
          </button>
        )}
